...
# Download ADNI data from https://ida.loni.usc.edu/home/projectPage.jsp?project=ADNI&page=HOME&subPage=OVERVIEW_PR

# SETUP - Install dcm2niix, numpy, os, shutil, glob

# The given code is for subjects with LMCI. Rename 'LMCI' to 'control', 'patient', 'SMC', 'EMCI', 'MCI' based on the subjects
# Update output folder path in 'Output main' - Line 18
# Update input folder path in 'for' loop - Line 25
...
for count, subject in enumerate(glob.glob("/home/data_fmriprep/ADNI_LMCI/*")):
    ...
    if niilength<3:
        ...
        for func_json in glob.glob(f"{Outputbase}/func/*.json"):
            os.rename(func_json,(f"{Outputfunc}sub-LMCI{modified_subname}_task-resting_run-1_bold.json"))
        for func_nii in glob.glob(f"{Outputbase}/func/*.nii.gz"):
            os.rename(func_nii,(f"{Outputfunc}sub-LMCI{modified_subname}_task-resting_run-1_bold.nii.gz"))
    elif niilength>2:
        #ANAT files
        for anat_extrajson in glob.glob(f"{Outputbase}/anat/*a.json"):
            shutil.move(anat_extrajson,(f"{Outputmain}sub-{modified_subname}_T1w.json"))
        for anat_extranii in glob.glob(f"{Outputbase}/anat/*a.nii.gz"):
            shutil.move(anat_extranii,(f"{Outputmain}sub-{modified_subname}_T1w.nii.gz"))
        ...
        for func_extrajson in glob.glob(f"{Outputbase}/func/*a.json"):
            shutil.move(func_extrajson,(f"{Outputmain}sub-{modified_subname}_task-resting_run-1_bold.json"))
        for func_extranii in glob.glob(f"{Outputbase}/func/*a.nii.gz"):
            shutil.move(func_extranii,(f"{Outputmain}sub-{modified_subname}_task-resting_run-1_bold.nii.gz"))
        for func_json2 in glob.glob(f"{Outputbase}/func/*.json"):
            os.rename(func_json2,(f"{Outputfunc}sub-LMCI{modified_subname}_task-resting_run-1_bold.json"))
        for func_nii2 in glob.glob(f"{Outputbase}/func/*.nii.gz"):
            os.rename(func_nii2,(f"{Outputfunc}sub-LMCI{modified_subname}_task-resting_run-1_bold.nii.gz"))        
